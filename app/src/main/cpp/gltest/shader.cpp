...
static void _printShaderLog(GLuint shader) {
    ...
    LOGI("*** Getting info log for shader %u", shader);
...
static void _printProgramLog(GLuint program) {
    char buf[2048];
    memset(buf, 0, sizeof(buf));
    LOGI("*** Getting info log for program %u", program);
...
void Shader::Compile() {
    ...
    LOGI("Shader name: %s", GetShaderName());
    ...
    if (status == 0) {
        LOGI("*** Shader program link failed, %d", status);
        ABORT_GAME;
    }
    LOGI("Program linking succeeded.");

    glUseProgram(mProgramH);
    mMVPMatrixLoc = glGetUniformLocation(mProgramH, "u_MVP");
    if (mMVPMatrixLoc < 0) {
        LOGI("*** Couldn't get shader's u_MVP matrix location from shader.");
        ABORT_GAME;
    }
    mPositionAttribLoc = glGetAttribLocation(mProgramH, "a_Position");
    if (mPositionAttribLoc < 0) {
        LOGI("*** Couldn't get shader's a_Position attribute location.");
        ABORT_GAME;
    }
    LOGI("Shader compilation/linking successful.");
    glUseProgram(0);
}

void Shader::BindShader() {
    if (mProgramH == 0) {
        LOGI("!!! WARNING: attempt to use shader before compiling.");
        LOGI("!!! Compiling now. Shader: %s", GetShaderName());
        Compile();
    }
    glUseProgram(mProgramH);
}
...
void TrivialShader::Compile() {
    ...
    if (mTintLoc < 0) {
        LOGI("*** Couldn't get tint uniform location from shader.");
        ABORT_GAME;
    }
    UnbindShader();
}
